...
static	int				(*g_format_func[])(unsigned char *) =
			{choose_elf_format, macho64, macho32, pe32};
...
int	choose_format(unsigned char *content_file, char *filename)
{
	...
	{
		choose_arch_format(content_file, filename);
		return (1);
	}
	if (filename)
		ft_printf("%s:\n", filename);
	ret = g_format_func[format - 1](content_file);
	return (ret);
}
